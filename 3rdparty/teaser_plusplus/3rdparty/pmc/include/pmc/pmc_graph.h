...
public:
    ...
    void reduce_graph(vector<long long>& vs, vector<int>& es, int*& pruned,
                      int id, int& mc);
    ...
    double density() {
        return (double)num_edges() /
               (num_vertices() * (num_vertices() - 1.0) / 2.0);
    }
    ...
    void compute_cores();
    void induced_cores_ordering(vector<long long>& V, vector<int>& E,
                                int*& pruned);
